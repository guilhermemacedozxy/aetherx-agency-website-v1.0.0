              ...
              <div className="flex flex-col gap-6">
                {category.items.map((item, itemIdx) => (
                  <Link
                    key={item.title}
                    href="#servicos"
                    className={`flex items-center gap-2.5 ${idx === CATEGORIES.length - 1 ? "w-83" : "w-61.75"} h-11 rounded-[7px] hover:bg-black/3 transition-colors duration-150 group${itemIdx === category.items.length - 1 ? " mb-14" : ""}`}
                  >
                    <div
